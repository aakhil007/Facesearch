#This code includes Spherical Clustering with random initalization and Hashing 
#In This code we need to delare number of cluster (varible 'nclus' is used for that purpose)
#We took number of clusters = 10% of total number of tracks 
import numpy as np
import csv
import os
import cv2
...
nclus = 25 #This is equal to number of tracks
ntrack = track.shape[0] #This should be equal to number of tracks
list1=[]
b = 0
...
#Initilisation (randomly)
for i in range(nclus-1):
	list1.append(range(i*(nimg/nclus),(i+1)*(nimg/nclus)))
list1.append(range((nclus-1)*(nimg/nclus),nimg))
print array
...
writer=csv.writer(open('meanv_rand.csv','w')) #meanv_rand.csv will be the output file which contains "meanv" numpy array
writer.writerows(meanv) 
#Spherical K-means done with "nclus" clusters and the mean vectors are stored in "meanv_rand.csv" file

#Hashing 
hashed=np.zeros((nimg,nclus)) #Created a numpy array size of nimg * nclus
...
writer=csv.writer(open('hashed_rand.csv','w')) #Stored hashed vectors in 'hashed_rand.csv' file
writer.writerows(hashed)

#print hashed
#finding the mean of each track using hashed faces
#Let's call that hash_mean
hash_mean = np.zeros((ntrack,nclus))
b = 0
...
#print hash_mean
writer=csv.writer(open('hash_mean_rand.csv','w')) #Stored mean of each track obtained using hashed vectors of each face.
writer.writerows(hash_mean)
writer=csv.writer(open('hash_cov_rand.csv','w'))
cov_list = []
b = 0 
for i in range(nclus):
...
#print cov_list[0]
#The below part is for visulaising each cluster, where it creates nclus folders and each folder contains images related to that cluster.
fold=0
for i in list1:
	os.makedirs('./clusters_rand/'+'cluster'+str(fold))
	for j in i:
		cv2.imwrite('./clusters_rand/'+'cluster'+str(fold)+'/'+str(j)+'.png',array2[j].reshape(64,64))
	fold +=1
